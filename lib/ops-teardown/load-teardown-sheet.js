...
export const OPS_TEARDOWN_ORDER_COLUMNS =
  "id, ops_drag_report_order:metadata->ops_drag_report_order";
...
export async function fetchOpsTeardownOrderRow(supabase, leadId) {
  if (!isUuid(leadId)) return null;
  const result = await supabase
    .from("custom_ops_hub_leads")
    .select(OPS_TEARDOWN_ORDER_COLUMNS)
    .eq("id", leadId)
    .maybeSingle();
  if (result.error) throw new Error("Unable to load Ops Check order");
  return result.data ?? null;
}

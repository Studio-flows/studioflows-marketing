...
const ALLOWED_PLATFORM_HOSTS = new Set(["os.studioflows.co"]);
...
export function resolvePlatformRoot() {
  ...
  return normalizePlatformRoot(configured);
}

function normalizePlatformRoot(platformRoot) {
  try {
    const parsed = new URL(String(platformRoot).trim());
    if (parsed.protocol === "https:" && ALLOWED_PLATFORM_HOSTS.has(parsed.hostname)) {
      return parsed.origin;
    }
  } catch {
    // Fall through to the canonical platform root.
  }
...
export function buildOpsAuditBookUrl({
  ...
  if (!TENANT_SLUG_PATTERN.test(normalizedTenantSlug)) return null;
  return `${normalizePlatformRoot(platformRoot)}/s/${normalizedTenantSlug}/ops-audit/book?${params.toString()}`;
}

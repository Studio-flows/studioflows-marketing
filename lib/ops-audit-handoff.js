...
/** Paid-order teardown surface. Tokens stay in the URL fragment and are never sent in the initial request. */
/** @param {{ viewToken?: string, pdfToken?: string, emailToken?: string, siteOrigin?: string }} input */
export function buildOpsTeardownThankYouUrl({
  ...
  if (viewToken) fragment.set("view_token", viewToken);
  if (pdfToken) fragment.set("pdf_token", pdfToken);
  if (emailToken) fragment.set("email_token", emailToken);
  return `${origin}${OPS_TEARDOWN_PATH}${fragment.size ? `#${fragment.toString()}` : ""}`;
}
